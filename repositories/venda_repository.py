...
vendas_busca = [] # Lista para guardar os resultados de uma pesquisa.
...
class VendaRepository:
    ...
    def cadastrar_venda(self, venda_data: VendaCreate):
        ...
        venda = Venda(
            id = len(vendas) + 1,
            **venda_data.model_dump() # Transforma o objeto em dicionário.
        )

        vendas.append(venda)
        return venda
    def buscar_vendas(self, data_realizada, id_vendedor, id_cliente):
        if(data_realizada):
            for venda in vendas:
